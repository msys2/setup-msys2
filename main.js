...
class PackageCache {
  ...
  constructor(msysRootDir, input) {
    ...
    shasum.update([CACHE_FLUSH_COUNTER, input.release, input.update, input.pathtype, input.msystem, input.install, input.pacboy, input.usemainmirroronly].toString() + INSTALLER_CHECKSUM);
